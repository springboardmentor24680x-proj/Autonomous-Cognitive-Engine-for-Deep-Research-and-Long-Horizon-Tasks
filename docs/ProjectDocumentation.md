...
```
pip install langgraph langchain openai python-dotenv tavily-python streamlit pydantic requests
```

**Create a .env file and set API keys:**
```
...
**Open the browser and access the application at:**
```
http://localhost:8501
```
...
### 6.1 Planner Agent (`agents/planner_agent.py`)
...
### 6.2 Research & Search Agents (`agents/research_agent.py` / `agents/search_agent.py`)
...
### 6.3 Summarizer Agent (`agents/summarizer_agent.py`)
...
### 6.4 Virtual File System (`memory/vfs.py`)
...
## 8. UI Behavior and Typical Flow
...
![alt text](<Screenshot 2026-01-20 214139.png>)
